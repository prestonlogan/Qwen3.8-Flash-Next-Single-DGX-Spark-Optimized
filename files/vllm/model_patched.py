...
class Qwen3_8FlashNextForConditionalGeneration(
    ...
    def checkpoint_weight_filter(name: str) -> bool:
        # The target never consumes embedded MTP tensors. When PLE CPU
        # offload is active, its packed mmap owns the n-gram table; only
        # the NVFP4 global scale is materialized by the GPU placeholder.
        if name.startswith("mtp.") or ".mtp." in name:
            return False
        if envs.VLLM_PLE_CPU_OFFLOAD and ".ple.ple_embedding." in name:
            return name.endswith(".ngram_embedding.weight_scale_2")
        return True
