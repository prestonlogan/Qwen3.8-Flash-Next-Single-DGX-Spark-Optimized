...
class RoutedExperts(PluggableLayer):
    ...
    def load_weights(
        ...
        mapping_index: dict[
            str, list[tuple[int, tuple[str, str, int, str]]]
        ] = {}
        for mapping_order, mapping in enumerate(expert_mapping):
            mapping_index.setdefault(mapping[1], []).append(
                (mapping_order, mapping)
            )

        def indexed_expert_mapping(
            qual_name: str,
        ) -> list[tuple[str, str, int, str]]:
            # Every checkpoint-side mapping produced by
            # build_expert_params_mapping starts at an `experts`
            # component. Generate only dot-boundary prefixes from
            # that component, then restore original mapping order.
            # This preserves fused/replicated multi-destination
            # mappings without scanning all experts per tensor.
            parts = qual_name.split(".")
            candidates: dict[int, tuple[str, str, int, str]] = {}
            for start, component in enumerate(parts):
                ...
                for end in range(start, len(parts)):
                    if end > start:
                        key += f".{parts[end]}"
                    for lookup_key in (key, f"{key}."):
                        for mapping_order, mapping in mapping_index.get(
                            lookup_key, ()
                        ):
                            candidates[mapping_order] = mapping
            return [candidates[i] for i in sorted(candidates)]

        def try_batched_modelopt_nvfp4(
            buffered_weights: list[tuple[str, torch.Tensor]],
        ) -> tuple[list[str], float, float, int] | None:
            # Spark specialization: collapse thousands of tiny
            # CPU-to-CUDA assignments into one bounded copy per
            # parameter/projection group. Every guard below must
            # hold or the untouched generic loader handles the call.
            if not buffered_weights:
                return None
            if os.getenv("QWEN38_BATCHED_EXPERT_LOAD", "0") != "1":
                return None
            ...
            if getattr(parallel, "ep_size", 1) != 1:
                return None
            ...
            groups: dict[
                tuple[str, str],
                list[tuple[int, torch.Tensor, torch.nn.Parameter]],
            ] = {}
            for expert_name, loaded_weight in buffered_weights:
                if loaded_weight.device.type != "cpu":
                    return None
                qual_name = f"{self.layer_name}.{expert_name}"
                candidates = indexed_expert_mapping(qual_name)
                if len(candidates) != 1:
                    return None
                raw_param_name, weight_name, expert_id, shard_id = (
                    candidates[0]
                )
                if self._map_global_expert_id_to_local_expert_id(
                    expert_id
                ) != expert_id:
                    return None
                mapped_name = qual_name.replace(weight_name, raw_param_name)
                param_name = mapped_name.removeprefix(
                    f"{self.layer_name}."
                )
                if param_name not in allowed_params:
                    return None
                param = getattr(self, param_name, None)
                if param is None or getattr(param, "is_transposed", False):
                    return None
                groups.setdefault((param_name, shard_id), []).append(
                    (expert_id, loaded_weight, param)
                )

            # Validate every destination before modifying parameters.
            plans: list[tuple[
                str, torch.Tensor, list[torch.Tensor], bool
            ]] = []
            for (param_name, shard_id), records in groups.items():
                records.sort(key=lambda record: record[0])
                expert_ids = [record[0] for record in records]
                start = expert_ids[0]
                if expert_ids != list(range(start, start + len(records))):
                    return None
                tensors = [record[1] for record in records]
                tensor_layouts = {
                    (tensor.shape, tensor.dtype) for tensor in tensors
                }
                if len(tensor_layouts) != 1:
                    return None
                param = records[0][2]
                scalar = param_name.endswith((
                ...
                if scalar:
                    ...
                    if param_name.startswith("w13_"):
                        ...
                        destination = param.data[
                            start : start + len(records), shard_index
                        ]
                    elif shard_id == "w2":
                        destination = param.data[
                            start : start + len(records)
                        ]
                    else:
                        return None
                    ...
                    if param_name.startswith("w13_"):
                        shard_size = param.data.shape[1] // 2
                        shard_start = 0 if shard_id == "w1" else shard_size
                        destination = destination.narrow(
                            1, shard_start, shard_size
                        )
                    elif shard_id != "w2":
                        return None
                    expected_shape = (len(records), *tensors[0].shape)
                if tuple(destination.shape) != expected_shape:
                    return None
                plans.append((param_name, destination, tensors, scalar))

            staging_seconds = copy_seconds = 0.0
            loaded_params: list[str] = []
            for param_name, destination, tensors, scalar in plans:
                staging_started = time.perf_counter()
                if scalar:
                    staged = torch.stack(
                        [tensor.reshape(()) for tensor in tensors]
                    )
                else:
            ...
            return loaded_params, staging_seconds, copy_seconds, len(plans)

        diagnostics = os.getenv("QWEN38_WEIGHT_LOAD_DIAGNOSTICS", "0") == "1"
        diagnostic_started = time.perf_counter()
        diagnostic_tensors = diagnostic_candidates = 0
        diagnostic_matches = diagnostic_loader_calls = 0
        diagnostic_weight_loader_seconds = 0.0
        buffered_weights: list[tuple[str, torch.Tensor]] | None = None
        if (
            self.quant_method.__class__.__name__
            == "ModelOptNvFp4FusedMoE"
        ):
            buffered_weights = list(weights)
            batch_result = try_batched_modelopt_nvfp4(buffered_weights)
            if batch_result is not None:
                loaded_params, staging_s, copy_s, batch_copies = batch_result
                if diagnostics:
                    logger.info(
                        "RoutedExperts batched load layer=%s tensors=%d "
                        "batch_copies=%d staging_s=%.4f copy_s=%.4f total_s=%.4f",
                        self.layer_name, len(buffered_weights), batch_copies,
                        staging_s, copy_s, staging_s + copy_s,
                    )
                yield from loaded_params
                return
        weights_to_load = (
            buffered_weights if buffered_weights is not None else weights
        )
        for expert_name, loaded_weight in weights_to_load:
            if diagnostics:
                diagnostic_tensors += 1
            qual_name = f"{self.layer_name}.{expert_name}"
            # Fused expert weights can be identified by their 3D tensors
            is_fused = loaded_weight.dim() == 3
            matched = False
            for param_name, weight_name, expert_id, shard_id in indexed_expert_mapping(qual_name):
                if diagnostics:
                ...
                for expert_id, loaded_expert in enumerate(loaded_experts, start=start):
                    load_started = time.perf_counter() if diagnostics else 0.0
                    try:
                        ...
                        if diagnostics:
                            diagnostic_loader_calls += 1
                            diagnostic_weight_loader_seconds += (
        ...
        if diagnostics:
            ...
            logger.info(
                "RoutedExperts load diagnostics layer=%s tensors=%d "
                "mapping_entries=%d candidate_checks=%d matches=%d "
                "weight_loader_calls=%d routing_and_preparation_s=%.4f "
                "weight_loader_s=%.4f total_s=%.4f",
                self.layer_name, diagnostic_tensors, len(expert_mapping),
                diagnostic_candidates, diagnostic_matches,
                diagnostic_loader_calls,
                max(0.0, total_seconds - diagnostic_weight_loader_seconds),
                diagnostic_weight_loader_seconds, total_seconds,
            )
