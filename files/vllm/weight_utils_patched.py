...
def safetensors_weights_iterator(
    ...
    *,
    ...
    reduces storage I/O for MoE models under EP. A model-provided
    *weight_name_filter* is applied at the same metadata-only boundary.
    """
    ...
    def diagnostic_io_counters() -> dict[str, int]:
    ...
    diagnostic_usage_start = (
        resource.getrusage(resource.RUSAGE_SELF) if diagnostics else None
    )
    diagnostic_io_start = diagnostic_io_counters()
    ...
    for st_file in tqdm(
        ...
        if safetensors_load_strategy == "eager":
            ...
            with safe_open(st_file, framework="pt") as f:
                ...
                names = f.keys()
                keys_seconds = time.perf_counter() - keys_started
                seen = kept = skipped = retained_bytes = 0
                filter_seconds = get_tensor_seconds = consumer_seconds = 0.0
                for name in names:
                    seen += 1
                    ...
                    param = f.get_tensor(name)
                    get_tensor_seconds += time.perf_counter() - get_started
                    retained_bytes += param.numel() * param.element_size()
            ...
            row = {
                "seen": seen, "kept": kept, "skipped": skipped,
                "bytes": retained_bytes, "open": open_seconds,
                "keys": keys_seconds, "filter": filter_seconds,
                "get": get_tensor_seconds, "consumer": consumer_seconds,
                "total": total_seconds,
            }
            diagnostic_rows.append(row)
            logger.info(
                "Weight-load diagnostics shard=%s seen=%d kept=%d skipped=%d "
                "retained_gib=%.4f open_s=%.4f keys_s=%.4f filter_s=%.4f "
                "get_tensor_s=%.4f consumer_s=%.4f total_s=%.4f",
                os.path.basename(st_file), seen, kept, skipped,
                retained_bytes / 1024**3, open_seconds, keys_seconds,
                filter_seconds, get_tensor_seconds, consumer_seconds,
                total_seconds,
            )

    if diagnostics and diagnostic_rows:
        logger.info(
            "Weight-load diagnostics summary shards=%d seen=%d kept=%d skipped=%d "
            "retained_gib=%.4f open_s=%.4f keys_s=%.4f filter_s=%.4f "
            "get_tensor_s=%.4f consumer_s=%.4f total_s=%.4f",
            len(diagnostic_rows),
            sum(int(row["seen"]) for row in diagnostic_rows),
            sum(int(row["kept"]) for row in diagnostic_rows),
            sum(int(row["skipped"]) for row in diagnostic_rows),
            sum(int(row["bytes"]) for row in diagnostic_rows) / 1024**3,
            sum(float(row["open"]) for row in diagnostic_rows),
            sum(float(row["keys"]) for row in diagnostic_rows),
            sum(float(row["filter"]) for row in diagnostic_rows),
            sum(float(row["get"]) for row in diagnostic_rows),
            sum(float(row["consumer"]) for row in diagnostic_rows),
            sum(float(row["total"]) for row in diagnostic_rows),
        )
        diagnostic_usage_end = resource.getrusage(resource.RUSAGE_SELF)
        diagnostic_io_end = diagnostic_io_counters()
        assert diagnostic_usage_start is not None
        logger.info(
            "Weight-load process diagnostics user_cpu_s=%.4f system_cpu_s=%.4f "
            "minor_faults=%d major_faults=%d read_gib=%.4f rchar_gib=%.4f",
            diagnostic_usage_end.ru_utime - diagnostic_usage_start.ru_utime,
            diagnostic_usage_end.ru_stime - diagnostic_usage_start.ru_stime,
            diagnostic_usage_end.ru_minflt - diagnostic_usage_start.ru_minflt,
            diagnostic_usage_end.ru_majflt - diagnostic_usage_start.ru_majflt,
            (diagnostic_io_end.get("read_bytes", 0)
             - diagnostic_io_start.get("read_bytes", 0)) / 1024**3,
            (diagnostic_io_end.get("rchar", 0)
             - diagnostic_io_start.get("rchar", 0)) / 1024**3,
        )
