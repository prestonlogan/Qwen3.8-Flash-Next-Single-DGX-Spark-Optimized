...
# S2: probabilistic MTP drafting with the fast reduced-vocab draft head. /exp/probdraft.txt: "on K" | "off" | "status"
...
if arg[0] == "on":
    ...
    def sample_draft(hidden_states, positions, idx_mapping, temperature, seeds, draft_step, draft_logits):
        return gumbel_sample(fast_full_logits(hidden_states, temperature, idx_mapping), idx_mapping, temperature, seeds, positions + 1,
    ...
    ST["on"] = (K, TOPP, MULT)
    recapture()
    out.append(f"probdraft ON K={K} topp={TOPP} mult={MULT}; draft_logits {tuple(sp.draft_logits.shape)} {sp.draft_logits.dtype}")
elif arg[0] == "off":
