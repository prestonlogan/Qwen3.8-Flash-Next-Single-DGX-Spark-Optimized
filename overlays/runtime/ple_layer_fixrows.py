    ...
    def forward_impl(  # type: ignore[override]
        ...
        if output_buffer is not None:
            ...
            if packed is not None:
                ...
                # EXP fix: output is a strided slice of the 2560-wide buffer, so
                # reshape(-1, row_width) is a COPY for num_tokens > 1 and only row 0
                # ever landed. Gather contiguously, then copy into the strided view.
                rows = torch.index_select(packed, 0, ids)
                output.view(num_tokens, -1, row_width).copy_(
                    rows.view(num_tokens, -1, row_width)
                )
                return output
