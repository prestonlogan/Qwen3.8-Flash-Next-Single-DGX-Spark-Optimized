...
# ---------------------------------------------------------------- EXP zero-copy
# EXP: when /dev/shm/exp_ple_zc exists and header[0]==1, write PLE rows straight into
# that host-shared buffer (the GPU process maps it with cudaHostRegister and copies it in-graph
# after the done-flag spin). The PLE process then does no GPU work (no DMA, no stream sync).
_EXP_ZC = {"t": None, "hdr": None, "fence": None, "tries": 0}
_EXP_ZC_PATH = "/dev/shm/exp_ple_zc"
_EXP_ZC_HDR = 4096


def _exp_zc_get(like: torch.Tensor):
    z = _EXP_ZC
    if z["t"] is None:
        z["tries"] += 1
        if z["tries"] % 64 != 1 or not os.path.exists(_EXP_ZC_PATH):
            return None
        import ctypes
        import numpy as np
        nbytes = like.numel() * like.element_size()
        if os.path.getsize(_EXP_ZC_PATH) < _EXP_ZC_HDR + nbytes:
            return None
        mm = np.memmap(_EXP_ZC_PATH, dtype=np.uint8, mode="r+")
        z["mm"] = mm
        z["hdr"] = torch.from_numpy(mm[:_EXP_ZC_HDR].view(np.int64))
        z["t"] = torch.from_numpy(mm[_EXP_ZC_HDR:_EXP_ZC_HDR + nbytes]).view(like.dtype).view(like.shape)
        lib = ctypes.CDLL("/exp/exp_dmb.so")
        z["fence"] = lib.exp_dmb
        logger.info("EXP PLE zero-copy buffer attached (%d B)", nbytes)
    return z
...
class PleOffloadRunner:
    ...
    def _load_weights(self) -> None:
        ...
        original_checkpoint_filter = getattr(
            model, "checkpoint_weight_filter", None
        )

        def offload_checkpoint_filter(weight_name: str) -> bool:
            if (
                original_checkpoint_filter is not None
                and not original_checkpoint_filter(weight_name)
            ):
                return False
            ...
            return bool(
                ...
                and mapped_name.startswith(offload_prefixes)
                and not (
                    packed_prefixes
                    and mapped_name.startswith(packed_prefixes)
                )
            )
        ...
        if isinstance(loader, DummyModelLoader):
            ...
            # get_all_weights reads this hook while constructing its
            # lazy source, before any checkpoint tensor is materialized.
            model.checkpoint_weight_filter = offload_checkpoint_filter
    ...
    def busy_loop(
        ...
        EXP: when the zero-copy header is attached and hdr[3]==1, the GPU process also
        publishes each request in hdr[16..20] (seq last, after a fence); this loop
        spin-polls that slot and handles whichever copy (shm or zmq) arrives first,
        dropping the duplicate by seq."""
        logger.info("Busy-loop started (EXP shm-request capable).")
        poller = zmq.Poller()
        poller.register(pull_socket, zmq.POLLIN)
        last = 0
        idle0 = time.monotonic()
        while not shutdown_event.is_set():
            z = _EXP_ZC if _EXP_ZC["t"] is not None else None
            if z is not None and int(z["hdr"][3]) == 1:
                hdr = z["hdr"]
                s = int(hdr[20])
                if s > last:
                    z["fence"]()
                    req = PleOffloadRequest(dp_rank=int(hdr[16]), num_tokens=int(hdr[17]),
                                            num_reqs=int(hdr[18]), seq=s)
                    last = s
                    hdr[21] = int(hdr[21]) + 1
                    self._handle_requests([req])
                    idle0 = time.monotonic()
                    continue
                try:
                    while True:
                        rq = _PLE_OFFLOAD_REQUEST_DECODER.decode(pull_socket.recv(zmq.NOBLOCK))
                        if rq.seq > last:
                            last = rq.seq
                            hdr[22] = int(hdr[22]) + 1
                            self._handle_requests([rq])
                except zmq.Again:
                    pass
                if time.monotonic() - idle0 > 0.005:
                    time.sleep(0.0002)
                continue
            ...
            requests = [q for q in requests if q.seq > last]
            if requests:
                last = max(q.seq for q in requests)
    ...
    def _handle_requests(self, requests: list[PleOffloadRequest]) -> None:
        ...
        if len(self._layers) == 1 and len(requests_by_dp) == 1:
            _ln = next(iter(self._layers)); _dp = next(iter(requests_by_dp))
            try:
                zc = _exp_zc_get(self._pinned_bufs[_dp][_ln])
            except Exception:
                logger.exception("EXP zero-copy attach failed; using DMA path")
                _EXP_ZC["tries"] = 2  # retry on the next 64-request boundary
                zc = None
        zt = zc
        if zc is not None:
            zc["hdr"][8] = _t_recv
        ...
        for layer_name, layer in self._layers.items():
            for dp_rank, request in requests_by_dp.items():
                ...
                result = layer.forward_impl(
                    ...
                    ngram_context,
                    output_buffer=(zc["t"] if zc is not None else self._pinned_bufs[dp_rank][layer_name]),
                )
                if zc is not None:
                    zc["hdr"][1] = result.shape[1] * result.element_size()
                    zc["hdr"][2] = result.shape[0]
                    continue
        ...
        for dp_rank, request in requests_by_dp.items():
            ...
            if zt is not None:
                zt["hdr"][9] = time.monotonic_ns()
            if zc is not None:
                zc["fence"]()
            for flag in flags:
                flag[0] = request.seq
            if zt is not None:
                zt["hdr"][10] = time.monotonic_ns(); zt["hdr"][11] = request.seq
